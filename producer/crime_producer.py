...
Pacing modlari (PRODUCER_MODE):
  fixed     - PRODUCER_RATE msg/saniye sabit hızda bas (varsayilan)
  speedup   - tarih farklarını PRODUCER_SPEEDUP ile sikistirilmis biçimde uygula
...
def _parse_date_to_epoch_ms(s: str) -> int:
    """'05/13/2023 11:30:00 AM' -> epoch ms (UTC)."""
    try:
...
def main() -> int:
    if not os.path.exists(CRIMES_PATH):
        sys.stderr.write(f"Crimes.csv bulunamadi: {CRIMES_PATH}\n")
        return 2
    ...
    print(
        f"[producer] broker={KAFKA_BROKER} topic={TOPIC} path={CRIMES_PATH} "
        f"mode={MODE} rate={RATE} speedup={SPEEDUP} max={MAX_RECORDS}",
        flush=True,
    )

    with open(CRIMES_PATH, newline="", encoding="utf-8") as fh:
        ...
        for row in reader:
            ...
            if event_ms == 0:
                # Tarihsiz satırı atla; downstream ML'de event_time gerekli
                continue
    ...
    producer.flush(30)
    print(
        f"[producer] done sent={sent} elapsed={time.time() - started_at:.1f}s",
        flush=True,
    )
    return 0
